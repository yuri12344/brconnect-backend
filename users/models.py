...
class Company(models.Model):
    ...
    zip_code = models.CharField(
        max_length=255, verbose_name="CEP", blank=True, null=True
    )
    phone = models.CharField(
        max_length=17, blank=True, null=True, verbose_name="Numero de telefone"
    )
    ...
    owner = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="company", verbose_name="Criador"
    )
    chave_pix = models.CharField(
        max_length=255, verbose_name="Chave Pix", blank=True, null=True
    )
    ...
    order_expiration_days = models.PositiveIntegerField(
        default=7, verbose_name="Dias para expiração do pedido não pago e não enviado"
    )
    ...
    whatsapp_service = models.CharField(
        max_length=255,
        choices=[("wppconnect", "WppConnect"), ("baileys", "Baileys")],
        default="wppconnect",
    )
...
class Region(BaseModel):
    ...
    cost = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="Custo frete"
    )
...
class Supplier(BaseModel):
    ...
    company = models.ForeignKey(
        ...
        verbose_name="Empresa",
        blank=True,
        null=True,  # Adicione blank e null aqui se a empresa pode ser nula
    )
    ...
    city = models.CharField(
        max_length=255, null=True, blank=True, verbose_name="Cidade"
    )
    state = models.CharField(
        max_length=2,
        choices=STATE_CHOICES,
    ...
    region = models.ForeignKey(
        Region, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="Regiao"
    )
    cnpj = models.CharField(max_length=255, verbose_name="CNPJ", blank=True, null=True)
    description = models.TextField(verbose_name="Descrição", blank=True, null=True)
    chave_pix = models.CharField(
...
class Customer(BaseModel):
    ...
    interaction_history = models.TextField(
        null=True, blank=True, verbose_name="Histórico de Interações"
    )
    favorite_product = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="loved_by",
        verbose_name="Produto Favorito",
    )
    purchase_history = models.ManyToManyField(
        "sales.Order",
        related_name="purchasing_customers",
        blank=True,
        verbose_name="Histórico de Pedidos",
    )
    preferences = models.ManyToManyField(
        "products.Product", blank=True, verbose_name="Preferências de Produto"
    )
    ...
    def calculate_score(self):
        self.score = (
            self.interactions.aggregate(total_score=models.Sum("score"))["total_score"]
            or 0
        )
        self.save()
...
class CustomerGroup(BaseModel):
    ...
    customers = models.ManyToManyField(
        Customer, related_name="groups", blank=True, verbose_name="Clientes"
    )
...
class Interaction(BaseModel):
    ...
    customers = models.ManyToManyField(
        Customer, related_name="interactions", verbose_name="Clientes", blank=True
    )
