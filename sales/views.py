...
class OrderManager:
    ...
    def create_or_update_order(self):
        if self.customer.has_order():
            ...
            """ Else, create it"""
            order = Order.create_order_from_whatsapp_order(self.whatsapp_order, self.customer, self.company)
        return order
...
class HandleOrder(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            Product.create_products_in_back_end_from_whatsapp_order(whatsapp_order)
            
            order_manager = OrderManager(customer, whatsapp_order, company, whatsapp_client)
